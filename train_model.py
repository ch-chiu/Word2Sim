...
if __name__ == '__main__':
    ...
    joblib.dump(knn_cv, "model/knncv_model.m")
    print("CV score: {:.2f}".format(knn_cv.cv_scores_.mean(axis=0).max()))

    # # Training WordMoversKNN model
    # knn = WordMoversKNN(W_embed=W_common, verbose=5, n_jobs=8)
    # logger.info("Starting training model.")
    # knn.fit(X_train, y_train)
    # joblib.dump(knn, "model/knn_model.m")

    results = knn_cv.predict(X_test)
